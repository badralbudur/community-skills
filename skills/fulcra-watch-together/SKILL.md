...
7. Enrich canonical titles to build the required feature arrays for the catalog. For accurate and structured enrichment, use the following methods in order of preference:
   - **TMDB API or OMDb API:** (Preferred) If the user can provide an API key, use `web_fetch` or `curl` to query TMDB/OMDb for clean, structured JSON containing genres, directors, cast, and keywords.
   ...
   Cache provider IDs, media type, year, features, runtime, language, creators, franchise, region availability, provenance, and confidence. Never silently merge ambiguous titles or remakes.
