...
#: ``2026-01-02 03:05PM UTC`` (see ``transport.parse_list_output``). This is the
...
def _normalize_ts(raw: Any) -> str:
    ...
    string (``2026-01-02T09:00:00Z``). Accepts ISO-8601 (the ``timestamp``
    frontmatter reconcile stamps) AND the store's list-style ``mtime``
    (``2026-01-02 03:05PM UTC``), the defensive fallback — so a task with no
